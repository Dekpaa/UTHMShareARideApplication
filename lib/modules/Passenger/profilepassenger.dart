...
class _PassengerProfilePageState extends State<PassengerProfilePage> {
  final PassengerService _service = PassengerService();
  ...
  // documents - only matric card (optional)
  ...
  Future<void> _uploadProfileImage() async {
    ...
    try {
      ...
      final path =
          'passenger_documents/$uid/profile_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<void> _uploadFileAndSave(
    ...
    try {
      final uid = FirebaseAuth.instance.currentUser!.uid;
      final path =
          'passenger_documents/$uid/$filenamePrefix${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<void> _removeMatric() async {
    final url = _matricCardUrl;
    if (url == null || url.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No matric card to remove')),
        );
      }
      return;
    }
    ...
    try {
      await _service.updateProfile({'matricCardUrl': ''});
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Matric card removed')),
        );
      }
      setState(() {
        _matricCardUrl = '';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to remove matric card: $e')),
        );
      }
    }
  }
                ...
                onPick: () async {
                  await _pickDoc(
                    onPicked: (f) async {
                      ...
                      await _uploadMatric(); // auto upload after pick
                    },
                  );
                },
